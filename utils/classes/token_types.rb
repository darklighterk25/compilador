class TokenTypes
  ...
  def initialize
    @RESERVED_WORDS = %w[bool do else end float for if integer main read repeat then until while write]
    @TOKEN_STYLE = {
        ...
        rw_bool: 3, rw_do: 3, rw_else: 3, rw_end: 3, rw_float: 3, rw_for: 3, rw_if: 3, rw_integer: 3, rw_main: 3,
        rw_read: 3, rw_repeat: 3, rw_then: 3, rw_until: 3, rw_while: 3, rw_write: 3, semicolon: 2, string: 6,
        subtraction: 4,
    }
    @TOKEN_TYPE = {
        ...
        multiplication: "Multiplicación", not_equal: "Distinto", op_brace: "Llave que abre",
        ...
        rw_for: "Palabra reservada (For)", rw_if: "Palabra reservada (If)", rw_integer: "Palabra reservada (Integer)",
        rw_main: "Palabra reservada (Main)", rw_read: "Palabra reservada (Read)",
        rw_repeat: "Palabra reservada (Repeat)", rw_then: "Palabra reservada (Then)",
        rw_until: "Palabra reservada (Until)", rw_while: "Palabra reservada (While)",
        rw_write: "Palabra reservada (Write)", semicolon: "Punto y coma", string: "Cadena", subtraction: "Resta"
    }
  end
